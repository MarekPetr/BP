 ...
 Tato sekce čerpá informace z oficiálních návodů pro architekturu \emph{Android}~\footnoteURL{https://developer.android.com/jetpack/docs/guide}. Jedná se o doporučenou architekturu pro vývoj mobilních aplikací a aplikace, které se týká tato práce je na ní postavena. Pro implementaci této architektury se využívá knihovny \emph{Android Jetpack}~\footnoteURL{https://developer.android.com/jetpack}.
    ...
    Existují dva základní architektonické principy. Jejich použití řeší problém kde uchovávat data a stav aplikace tak, aby byly odděleny od uživatelského rozhraní a aplikace byla dobře testovatelná a konzistentní.
        ...
        \begin{enumerate}
            ...
            \item \textbf{LiveData}~\cite{livedata} - třída, obsahující data. Je uzpůsobena ke komunikaci mezi aktivitou / fragmentem a \emph{ViewModelem}. 
        ...
        Následující kód demonstruje inicializaci \emph{LiveData}. Jedná se o wrapper, který může obsahovat jakékoliv data. Může tedy i implementovat kolekce, například seznam. Následující kód byl převzat z~\cite{livedata} a názorně ukazuje, jakým způsobem jsou LiveData inicializována. Z důvodu perzistence její hodnoty se tak děje v rámci \emph{ViewModelu}.

        \begin{lstlisting}[language=Java, caption=Inicializace LiveData]
        ...
        Hodnota \emph{LiveData} potřebná pro aktualizaci uživatelského rozhraní se získává pomocí \emph{Observeru}. Hodnotu je potřeba znát v rámci aktivity a proto se k odběru změn hodnoty \emph{LiveData} přihlásíme právě z ní.

        \begin{lstlisting}[language=Java, caption=Získání hodnoty LiveData]
