...
\newpage
\section{Git}
\emph{Git}~\cite{git} slouží zejména programátorům k verzování jejich práce i jejího sdílení s ostatními členy týmu. Jeho využití je široké a to zejména při využití rozšíření \emph{Git LFS}~\cite{git-lfs} nebo \emph{Git Annex}~\cite{git-annex}, která se zaměřují na práci s velkými soubory.
