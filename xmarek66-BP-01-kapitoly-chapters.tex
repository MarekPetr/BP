...
\begin{itemize}
    ...
    \item \textbf{onPause()}: Tímto voláním systém oznamuje, že uživatel opouští danou aktivitu. Volání této metody je rychlé a proto by se nemělo používat k ukládání dat uživatele, či provádění jiných časově náročných operací. Pro tento účel slouží metoda \emph{onStop()}.
    
    \item \textbf{onStop()}: Když už aktivita není pro uživatele viditelná, vstoupila do stavu \emph{Stopped} a systém zavolal metodu \emph{onStop()}. Při přechodu do tohoto stavu by aplikace měla uvolnit všechny zdroje, které nejsou potřeba když uživatel aplikaci právě nepoužívá. Dále se zde provádí dlouhé operace pro uložení stavu aktivity před jejím opuštěním.
